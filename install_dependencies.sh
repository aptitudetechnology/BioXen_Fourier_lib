...
pip3 install --upgrade \
    ...
    notebook \
    ipywidgets \
    questionary # Ensure questionary is installed here explicitly if not in requirements.txt
...
if command -v luarocks &> /dev/null; then
    ...
    echo "  ⚠️  LuaRocks: not found (LuaSocket and BioLib2D installation unavailable)"
fi
...
echo "  4. Launch Love2D visualization: love libs/biolib2d/ (if BioLib2D installed)"
echo ""
